...
namespace WebApplication9.Controllers
{
    public class BlogController : Controller
    {
        ...
        public PartialViewResult FeaturedPosts()
        {
            ...
            //6.post
            var posttitle6 = bm.GetAll().OrderByDescending(z => z.BlogID).Where(x => x.CategoryID == 6).Select(y => y.BlogTitle).FirstOrDefault();
            var postimage6 = bm.GetAll().OrderByDescending(z => z.BlogID).Where(x => x.CategoryID == 6).Select(y => y.BlogImage).FirstOrDefault();
            var blogdate6 = bm.GetAll().OrderByDescending(z => z.BlogID).Where(x => x.CategoryID == 6).Select(y => y.BlogDate).FirstOrDefault();
            ViewBag.posttitle6 = posttitle6;
            ViewBag.postimage6 = postimage6;
            ViewBag.blogdate6 = blogdate6;
            return PartialView();
        }
